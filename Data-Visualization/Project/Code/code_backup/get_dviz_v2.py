...
state_coordinates = {'AL': (-89.00,-84.37,35.50,29.50), 'AK': (173.50,-130.00,71.50,51.25),
                     ...
                     'CA': (-125.00,-112.80,42.70,31.53), 'CO': (-109.12,-102.00,41.00,37.00),
...
# 
def plot_efficiency_gap_seat(df, ax):
...
def plot_missed_seats_by_state(df, state_id, ax, print_title=True, savefile=False):
    ...
    ax.barh(0, republican_votes, height=0.3 ,align='center', left=democrat_votes, color=republican_red)
    ...
    ax.set_xlabel('Adjusted % of Votes', size=14)
...
def print_district_map(eresults, state, ax):
    ...
    doannotate = (state_seat_count < 20)
